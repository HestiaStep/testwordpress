...
function rara_academic_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'rara-academic' ),
...
function rara_academic_entry_footer() {
	...
	if ( 'post' === get_post_type() && is_single() ) {
		...
		if ( $categories_list && rara_academic_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="fa fa-folder-open" aria-hidden="true"></i>' . esc_html__( '%1$s', 'rara-academic' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'rara-academic' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="fa fa-tags" aria-hidden="true"></i>' . esc_html__( '%1$s', 'rara-academic' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
